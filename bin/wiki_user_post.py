#!/usr/bin/env python3
"""
Append a message to a MediaWiki user page.

Examples:
  python3 wiki_user_post.py "ExampleUser" --message "Hello!" --credentials tests/inputs/wiki_credentials.json
  python3 wiki_user_post.py "User:ExampleUser" --message-file ./note.txt --summary "Automated note" --credentials tests/inputs/wiki_credentials.json
"""
...
def main() -> int:
    ...
    ap.add_argument("--api", default="https://en.wikipedia.org/w/api.php", help="MediaWiki API endpoint.")
    ap.add_argument("--message", help="Message to append to the user page.")
    ap.add_argument("--message-file", help="File containing the message to append.")
    ap.add_argument(
        "--summary",
        default="Automated note from wiki_user_post.py",
    ...
    with requests.Session() as s:
        s.headers.update(
            {"User-Agent": "AntMerrillWikiTool/0.4 (user post; automation)"}
        )
        login(s, args.api, creds["username"], creds["password"])
        new_rev = post_user_page(s, args.api, args.username, message, args.summary)
        if args.stdout:
            print(f"Posted to user page (rev {new_rev}).", flush=True)
